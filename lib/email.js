...
const FROM_EMAIL = process.env.FROM_EMAIL || 'CoachSet <notifications@coachset.pk>';
const APP_URL = process.env.APP_URL || 'https://coachset-pakistan.vercel.app';
...
<body>
  <div class="container">
    <div class="logo">CoachSet</div>
    <div class="card">
      ${content}
    </div>
    <div class="footer">
      CoachSet Pakistan · Verified Trainer Marketplace<br>
...
export const email = {
  ...
  newLead: async ({ trainerEmail, trainerName, clientName, clientGoal, clientMessage, leadId }) => {
    const subject = `New lead from ${clientName} · CoachSet`;
    const html = baseTemplate(`
      <div class="heading">You have a new lead, ${trainerName}</div>
      <div class="text"><strong class="highlight">${clientName}</strong> submitted an inquiry through CoachSet.</div>
      ...
      <div class="text">Responding quickly improves your ranking on CoachSet.</div>
    `);
    const text = `New lead from ${clientName} on CoachSet. Goal: ${clientGoal}. View: ${APP_URL}/trainer/dashboard`;
  ...
  paymentVerified: async ({ clientEmail, clientName, trainerName, packageTitle }) => {
    ...
    const html = baseTemplate(`
      ...
      <div class="text">Your trainer's contact details are now unlocked. You can message them directly through CoachSet.</div>
  ...
  newMessage: async ({ toEmail, recipientName, senderName, senderRole, bookingTitle, messagePreview }) => {
    const subject = `New message from ${senderName} · CoachSet`;
    const html = baseTemplate(`
  ...
  trainerApproved: async ({ trainerEmail, trainerName }) => {
    const subject = `Your CoachSet profile is live`;
    const html = baseTemplate(`
      <div class="heading">You're approved, ${trainerName}</div>
      <div class="text">Your trainer profile has been verified and is now live on CoachSet.</div>
    ...
    `);
    const text = `Your CoachSet profile is now live. View: ${APP_URL}/trainer/dashboard`;
    return send({ to: trainerEmail, subject, html, text });
  },
  ...
  trainerPayoutPaid: async ({ trainerEmail, trainerName, amount, method, reference, receiptImage }) => {
    ...
    const html = baseTemplate(`
      ...
      <div class="text">Thank you for being a CoachSet trainer.</div>
    `);
  ...
  passwordReset: async ({ toEmail, token }) => {
    ...
    const subject = `Reset your CoachSet password`;
    const html = baseTemplate(`
      <div class="heading">Password reset requested</div>
      <div class="text">Click the button below to reset your CoachSet password. This link expires in 1 hour.</div>
      <a href="${resetUrl}" class="button">Reset Password</a>
      <div class="text">If you didn't request this, you can safely ignore this email.</div>
    `);
    const text = `Reset your CoachSet password: ${resetUrl} (expires in 1 hour)`;
    return send({ to: toEmail, subject, html, text });
  }
};
